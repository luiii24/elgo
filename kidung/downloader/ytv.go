...
func init() {
	x.Rahwana(&x.Gace{
		Name:   "ytv",
		Cmd:    []string{"ytv", "ytmp4", "ytvideo", "youtubemp4"},
		Tags:   "downloader",
		IsQuery:  true,
		Value: ".ytv https://youtu.be/ONiQqw0bsHo?si=VZUBKMVVeeeN-k69",
    ...
    urldl := resp["mp4"].(map[string]map[string]interface{})["360p"]["url"].(string)
     var yt Inp
    ...
    m.SendDoc(m.Msg.Info.Chat, urldl, &x.SendDocParams{Capt: "", Mime: "video/mp4", FileName: yt.Localized.Title +".mp4", Thumb: yt.Thumbnails.Maxres.URL})        
			m.React("✅")
		},
	})
}
